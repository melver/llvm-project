...
static QualType inferTypeFromSizeofExpr(const Expr *E) {
  ...
  if (const auto *UET = llvm::dyn_cast<UnaryExprOrTypeTraitExpr>(Arg)) {
...
static QualType inferTypeFromArithSizeofExpr(const Expr *E) {
  ...
  if (!QT.isNull())
    return QT;
  // The argument is an arithmetic expression. Only covers multiplication for
  // arrays for now. Other arithmetic patterns involving sizeof are less common.
  if (const auto *BO = llvm::dyn_cast<BinaryOperator>(Arg)) {
    if (BO->getOpcode() == BO_Mul) {
      QT = inferTypeFromSizeofExpr(BO->getLHS());
      if (!QT.isNull())
        return QT;
      QT = inferTypeFromSizeofExpr(BO->getRHS());
      if (!QT.isNull())
        return QT;
    }
  }
...
static QualType inferTypeFromVarInitSizeofExpr(const Expr *E) {
  const Expr *Arg = E->IgnoreParenImpCasts();
  if (const auto *DRE = llvm::dyn_cast<DeclRefExpr>(Arg)) {
...
void CodeGenFunction::EmitAllocPartitionHint(llvm::CallBase *CB,
  ...
  if (!AllocType.isNull())
    EmitAllocPartitionHint(CB, AllocType);
}
  ...
  if (auto *CalleeDecl = dyn_cast_or_null<FunctionDecl>(TargetDecl)) {
    ...
    if (CalleeDecl->hasAttr<RestrictAttr>() ||
      ...
      if (SanOpts.has(SanitizerKind::AllocPartition)) {
        ...
        EmitAllocPartitionHint(LocalCallOrInvoke, E);
      }
    }
  }
  if (CallOrInvoke)
