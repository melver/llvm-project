// RUN: %clang_cc1    -fsanitize=alloc-token -fsanitize-alloc-token-max=2147483647 -triple x86_64-linux-gnu -x c -emit-llvm %s -o - | FileCheck %s
// RUN: %clang_cc1 -O -fsanitize=alloc-token -fsanitize-alloc-token-max=2147483647 -triple x86_64-linux-gnu -x c -emit-llvm %s -o - | FileCheck %s
...
void *test_malloc_use() {
  // CHECK: call{{.*}} ptr @__alloc_token_malloc(i64 noundef 4, i64 {{[1-9][0-9]*}})
...
void *test_struct_allocation() {
  // CHECK: call{{.*}} ptr @__alloc_token_malloc(i64 noundef 32, i64 {{[1-9][0-9]*}})
...
void test_multiple_allocs() {
  // CHECK: call{{.*}} ptr @__alloc_token_malloc(i64 noundef 40, i64 {{[1-9][0-9]*}})
  // CHECK: call{{.*}} ptr @__alloc_token_calloc(i64 noundef 4, i64 noundef 8, i64 {{[1-9][0-9]*}})
