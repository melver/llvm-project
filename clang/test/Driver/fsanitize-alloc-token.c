...
// RUN: %clang --target=x86_64-linux-gnu -fsanitize=alloc-token -fsanitize-alloc-token-max=4294967295 %s -### 2>&1 | FileCheck -check-prefix=CHECK-MAX %s
// CHECK-MAX: "-fsanitize-alloc-token-max=4294967295"
// RUN: not %clang --target=x86_64-linux-gnu -fsanitize=alloc-token -fsanitize-alloc-token-max=0 %s -### 2>&1 | FileCheck -check-prefix=CHECK-INVALID-MAX %s
// RUN: not %clang --target=x86_64-linux-gnu -fsanitize=alloc-token -fsanitize-alloc-token-max=-1 %s -### 2>&1 | FileCheck -check-prefix=CHECK-INVALID-MAX %s
