// Check -fsanitize=alloc-partition TypeHashPointerSplit mode with only 2
// partitions so we effectively only test the contains-pointer logic.
//
// RUN: %clang_cc1    -fsanitize=alloc-partition -fsanitize-alloc-partition-max=2 -triple x86_64-linux-gnu -std=c++20 -emit-llvm %s -o - | FileCheck %s
// RUN: %clang_cc1 -O -fsanitize=alloc-partition -fsanitize-alloc-partition-max=2 -triple x86_64-linux-gnu -std=c++20 -emit-llvm %s -o - | FileCheck %s
...
int **test_malloc_ptr() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 8, i64 1)
  int **a = (int **)malloc(sizeof(int*));
  *a = nullptr;
  return a;
}
...
int *test_new_int() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 4, i64 0){{.*}} !alloc_partition_hint
  return new int;
}

// CHECK-LABEL: @_Z20test_new_ulong_arrayv(
unsigned long *test_new_ulong_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 80, i64 0){{.*}} !alloc_partition_hint
  return new unsigned long[10];
}

// CHECK-LABEL: @_Z12test_new_ptrv(
int **test_new_ptr() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 8, i64 1){{.*}} !alloc_partition_hint
  return new int*;
}

// CHECK-LABEL: @_Z18test_new_ptr_arrayv(
int **test_new_ptr_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 80, i64 1){{.*}} !alloc_partition_hint
  return new int*[10];
}
...
void *test_malloc_struct_with_ptr() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 16, i64 1){{.*}} !alloc_partition_hint
  return malloc(sizeof(ContainsPtr));
}

// CHECK-LABEL: @_Z33test_malloc_struct_array_with_ptrv(
void *test_malloc_struct_array_with_ptr() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 160, i64 1){{.*}} !alloc_partition_hint
...
void *test_malloc_with_ptr_sizeof_var(int x) {
  ...
  size *= x;
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef %{{.*}}, i64 1){{.*}} !alloc_partition_hint
  return malloc(size);
}

// CHECK-LABEL: @_Z29test_malloc_with_ptr_castonlyv(
ContainsPtr *test_malloc_with_ptr_castonly() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 4096, i64 1){{.*}} !alloc_partition_hint
...
ContainsPtr *test_operatornew_struct_with_ptr() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 16, i64 1){{.*}} !alloc_partition_hint
...
ContainsPtr *test_operatornew_struct_array_with_ptr() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 160, i64 1){{.*}} !alloc_partition_hint
...
ContainsPtr *test_new_struct_array_with_ptr() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 160, i64 1){{.*}} !alloc_partition_hint
...
TestClass *test_new_class() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 64, i64 0){{.*}} !alloc_partition_hint
  return new TestClass();
}

// CHECK-LABEL: @_Z20test_new_class_arrayv(
TestClass *test_new_class_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 648, i64 0){{.*}} !alloc_partition_hint
...
VirtualTestClass *test_new_virtual_class() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 72, i64 1){{.*}} !alloc_partition_hint
  return new VirtualTestClass();
}

// CHECK-LABEL: @_Z28test_new_virtual_class_arrayv(
VirtualTestClass *test_new_virtual_class_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 728, i64 1){{.*}} !alloc_partition_hint
...
MyStructUintptr *test_uintptr_isptr() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 16, i64 1)
...
uptr *test_uintptr_isptr2() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 8, i64 1)
  return new uptr;
}
