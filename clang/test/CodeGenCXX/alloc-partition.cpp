// RUN: %clang_cc1    -fsanitize=alloc-partition -fsanitize-alloc-partition-max=2147483647 -triple x86_64-linux-gnu -std=c++20 -fexceptions -fcxx-exceptions -emit-llvm %s -o - | FileCheck %s
// RUN: %clang_cc1 -O -fsanitize=alloc-partition -fsanitize-alloc-partition-max=2147483647 -triple x86_64-linux-gnu -std=c++20 -fexceptions -fcxx-exceptions -emit-llvm %s -o - | FileCheck %s
...
void test_malloc_like() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 4, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = malloc(sizeof(int));
  // CHECK: call{{.*}} ptr @__alloc_partition_calloc(i64 noundef 3, i64 noundef 4, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = calloc(3, sizeof(int));
  // CHECK: call{{.*}} ptr @__alloc_partition_realloc(ptr noundef {{[^,]*}}, i64 noundef 8, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = realloc(sink, sizeof(long));
  // CHECK: call{{.*}} ptr @__alloc_partition_reallocarray(ptr noundef {{[^,]*}}, i64 noundef 5, i64 noundef 8, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = reallocarray(sink, 5, sizeof(long));
  // CHECK: call{{.*}} align 128{{.*}} ptr @__alloc_partition_aligned_alloc(i64 noundef 128, i64 noundef 4, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = aligned_alloc(128, sizeof(int));
  // CHECK: call{{.*}} ptr @__alloc_partition_memalign(i64 noundef 16, i64 noundef 4, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  sink = memalign(16, sizeof(int));
  // CHECK: call{{.*}} ptr @__alloc_partition_valloc(i64 noundef 4, i64 {{[1-9][0-9]*}}), !alloc_partition_hint
  sink = valloc(sizeof(int));
  // CHECK: call{{.*}} ptr @__alloc_partition_pvalloc(i64 noundef 4, i64 {{[1-9][0-9]*}}), !alloc_partition_hint
  sink = pvalloc(sizeof(int));
  // FIXME: Should not be partition ID 0!
  // CHECK: call{{.*}} i32 @__alloc_partition_posix_memalign(ptr noundef {{[^,]*}}, i64 noundef 64, i64 noundef 4, i64 0)
  posix_memalign(&sink, 64, sizeof(int));
}

// CHECK-LABEL: @_Z15test_malloc_usev(
void *test_malloc_use() {
  // CHECK: call{{.*}} ptr @__alloc_partition_malloc(i64 noundef 4, i64 {{[1-9][0-9]*}})
...
void test_operator_new() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 4, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
...
void test_operator_new_nothrow() {
  // CHECK: call {{.*}} ptr @__alloc_partition_ZnwmRKSt9nothrow_t(i64 noundef 4, ptr {{.*}} @_ZSt7nothrow, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
...
int *test_new_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 40, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  return new int[10];
}
...
int *test_new_array_nothrow() {
  // CHECK: call {{.*}} ptr @__alloc_partition_ZnamRKSt9nothrow_t(i64 noundef 40, ptr {{.*}} @_ZSt7nothrow, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  return new (std::nothrow) int[10];
}

// CHECK-LABEL: @_Z15no_sanitize_newv(
__attribute__((no_sanitize("alloc-partition"))) int *no_sanitize_new() {
...
void test_size_returning_new() {
  // FIXME: This should not be partition ID 0!
  // CHECK: call { ptr, i64 } @__alloc_partition_size_returning_new(i64 noundef 8, i64 0)
  // CHECK: call { ptr, i64 } @__alloc_partition_size_returning_new_hot_cold(i64 noundef 8, i8 noundef zeroext 1, i64 0)
  // CHECK: call { ptr, i64 } @__alloc_partition_size_returning_new_aligned(i64 noundef 8, i64 noundef 32, i64 0)
  // CHECK: call { ptr, i64 } @__alloc_partition_size_returning_new_aligned_hot_cold(i64 noundef 8, i64 noundef 32, i8 noundef zeroext 1, i64 0)
...
TestClass *test_exception_handling_new() {
  try {
    // CHECK: invoke {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 72, i64 {{[1-9][0-9]*}})
    // CHECK-NEXT: !alloc_partition_hint
...
TestClass *test_new_class() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znwm(i64 noundef 72, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
  TestClass *obj = new TestClass();
  obj->data[0] = 42;
  return obj;
}
...
TestClass *test_new_class_array() {
  // CHECK: call {{.*}} ptr @__alloc_partition_Znam(i64 noundef 728, i64 {{[1-9][0-9]*}}){{.*}} !alloc_partition_hint
