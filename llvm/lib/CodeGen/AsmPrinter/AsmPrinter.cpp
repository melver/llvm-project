...
void AsmPrinter::emitPCSections(const MachineFunction &MF) {
  ...
  auto EmitForMD = [&](const MDNode &MD, ArrayRef<const MCSymbol *> Syms,
                       bool Deltas) {
    // Expect the first operand to be a section name. After that, any number of
    ...
    for (const MDOperand &MDO : MD.operands()) {
      if (auto *C = dyn_cast<ConstantAsMetadata>(MDO)) {
        emitGlobalConstant(F.getParent()->getDataLayout(), C->getValue());
      } else {
        SwitchSection(cast<MDString>(MDO)->getString());
...
void AsmPrinter::emitFunctionBody() {
  ...
  if (needFuncLabels(*MF) || MAI->hasDotTypeDotSizeDirective()) {
