...
class ModuleSanitizerBinaryMetadata {
public:
  ModuleSanitizerBinaryMetadata(Module &M, SanitizerBinaryMetadataOptions Opts)
...
private:
  ...
  uint32_t getVersion() const {
    ...
    if (CM.hasValue() && (*CM == CodeModel::Medium || *CM == CodeModel::Large))
...
bool ModuleSanitizerBinaryMetadata::run() {
  MetadataInfoSet MIS;
...
void ModuleSanitizerBinaryMetadata::runOn(Function &F, MetadataInfoSet &MIS) {
  if (F.empty())
    return;
...
bool ModuleSanitizerBinaryMetadata::runOn(Instruction &I, MetadataInfoSet &MIS,
                                          MDBuilder &MDB) {
  ...
  if (Options.Atomics && I.mayReadOrWriteMemory()) {
    ...
    if (SSID.hasValue() && SSID.getValue() != SyncScope::SingleThread) {
      NumMetadataAtomics++;
...
GlobalVariable *
ModuleSanitizerBinaryMetadata::getSectionMarker(const Twine &MarkerName,
                                                Type *Ty) {
...
void ModuleSanitizerBinaryMetadata::createZeroSizedObjectInSection(
...
StringRef
ModuleSanitizerBinaryMetadata::getSectionName(StringRef SectionSuffix) {
  // FIXME: Other TargetTriple (req. string pool)
  return SectionSuffix;
}

Twine ModuleSanitizerBinaryMetadata::getSectionStart(StringRef SectionSuffix) {
  return "__start_" + SectionSuffix;
}

Twine ModuleSanitizerBinaryMetadata::getSectionEnd(StringRef SectionSuffix) {
  return "__stop_" + SectionSuffix;
}

} // namespace

ModuleSanitizerBinaryMetadataPass::ModuleSanitizerBinaryMetadataPass(
    SanitizerBinaryMetadataOptions Opts)
    : Options(std::move(Opts)) {}

PreservedAnalyses
ModuleSanitizerBinaryMetadataPass::run(Module &M, AnalysisManager<Module> &AM) {
  ModuleSanitizerBinaryMetadata Pass(M, Options);
  if (Pass.run())
