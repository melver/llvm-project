...
cl::opt<uint64_t> ClMaxTokens("alloc-token-max",
                              cl::desc("Maximum number of tokens per mode"),
...
private:
  ...
  /// with token ID.
  void replaceAllocationCall(CallBase *CB, LibFunc Func, uint64_t TokenID,
  ...
  /// Return the token ID from the allocation call.
  uint64_t getTokenForAllocation(const CallBase &CB,
...
bool AllocToken::instrumentFunction(Function &F) {
  ...
  for (auto &[CB, Func] : AllocCalls) {
    uint64_t TokenID = getTokenForAllocation(*CB, ORE);
    replaceAllocationCall(CB, Func, TokenID, TLI);
  }
