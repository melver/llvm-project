...
;
; RUN: opt < %s -passes=inferattrs,alloc-partition -S | FileCheck %s --check-prefixes=CHECK,DEFAULT
; RUN: opt < %s -passes=inferattrs,alloc-partition -alloc-partition-cover-strdup -S | FileCheck %s --check-prefixes=CHECK,COVER
...
define ptr @test_ignorable_allocation_functions(ptr %ptr) sanitize_alloc_partition {
entry:
  ; COVER:   call ptr @__alloc_partition_strdup(
  ; DEFAULT: call ptr @strdup(
  %ptr1 = call ptr @strdup(ptr %ptr)
  ; COVER:   call ptr @__alloc_partition_strdup(
  ; DEFAULT: call ptr @__strdup(
  %ptr2 = call ptr @__strdup(ptr %ptr)
  ; COVER:   call ptr @__alloc_partition_strndup(
