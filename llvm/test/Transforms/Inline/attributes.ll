...
define i32 @sanitize_alloc_partition_callee(i32 %i) sanitize_alloc_partition {
  ret i32 %i
}
...
define i32 @alwaysinline_sanitize_alloc_partition_callee(i32 %i) alwaysinline sanitize_alloc_partition {
  ret i32 %i
}
...
; Can inline sanitize_alloc_partition functions into a noattr function. The
; attribute is *not* viral, otherwise may break code.
define i32 @test_no_sanitize_alloc_partition(i32 %arg) {
; CHECK-LABEL: @test_no_sanitize_alloc_partition(
; CHECK-SAME: ) {
; CHECK-NOT: call
; CHECK: ret i32
entry:
  %x1 = call i32 @noattr_callee(i32 %arg)
  %x2 = call i32 @sanitize_alloc_partition_callee(i32 %x1)
  %x3 = call i32 @alwaysinline_callee(i32 %x2)
  %x4 = call i32 @alwaysinline_sanitize_alloc_partition_callee(i32 %x3)
  ret i32 %x4
}

; Can inline noattr functions into a sanitize_alloc_partition function. If
; inlinable noattr functions cannot be instrumented, they should be marked with
; explicit noinline.
define i32 @test_sanitize_alloc_partition(i32 %arg) sanitize_alloc_partition {
; CHECK-LABEL: @test_sanitize_alloc_partition(
; CHECK-SAME: ) [[SANITIZE_ALLOC_PARTITION:.*]] {
; CHECK-NOT: call
; CHECK: ret i32
entry:
  %x1 = call i32 @noattr_callee(i32 %arg)
...
; CHECK: attributes [[SANITIZE_ALLOC_PARTITION]] = { sanitize_alloc_partition }
